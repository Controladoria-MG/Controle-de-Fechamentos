...
Dois princípios, o 2º ajustado depois de o usuário notar uma confusão real ao vivo (2026-08-25 — clicar em "Simples Nacional" parecia "ligar" o Status "Fechado" sozinho):
1. **Um clique sempre limpa todos os outros filtros gerais antes de aplicar o(s) campo(s) daquele card/linha** (`limparFiltrosGerais()` dentro de `alternarFiltroEMostrarTabela()`/`filtrarPorVariosEMostrarTabela()`) — nunca acrescenta em cima do que já estava ativo. **Versão anterior era aditiva** (só mexia no próprio campo) porque isso deixava o total do card sempre exato mesmo dentro de um contexto já filtrado — mas criava uma confusão pior na prática: um filtro deixado de um clique anterior em outra parte da tela (ex.: Status "Fechado" clicado em algum outro card) continuava ativo "escondido" e se combinava com o próximo clique, dando a impressão de que o novo clique tinha ligado aquele filtro sozinho. Trocado a pedido explícito do usuário — agora um clique sempre mostra exatamente (e só) o que aquele card/linha representa, comportamento único e previsível, mesmo que isso signifique que clicar num card recalculado dentro de outro filtro (ex.: depois do ranking por Gerente) não preserva mais esse filtro anterior.
