...
1. **"Por Departamento" (Radar Fiscal) e "Por Segmento" (Análise de Balanço) usam o MESMO campo, `Segmento` (2026-09-11).** É a "2ª dimensão" de navegação (`QUEBRA_CONFIG_POR_TIPO[tipo].segunda`) — só o rótulo do card muda por Tipo de Relatório, o `chave` agrupado é `Segmento` nos dois. Antes o Radar Fiscal agrupava por `DeptoFiscal` (equipe fiscal interna — coluna `Departamento` no schema normalizado), mas essa coluna só distinguia Varejo/Indústria dentro da unidade SP (nas demais unidades tinha 1 valor só, e os clientes Holding/Serviços de SP ficavam escondidos dentro de EF-INDUSTRIA) — pedido do usuário ("mostrar Holding e Serviços também") trocou pra `Segmento`, que tem esses valores em todas as unidades. O campo `Departamento`/coluna `DeptoFiscal` foi **removido do schema normalizado** (não é mais gerado por `_normalizar_radar_fiscal`/`_normalizar_analise_balanco` nem lido por `normalizarLinha` no front) — já não sobrava nenhum outro uso dele. A coluna "Departamento" já tinha sido removida da tabela e do modal antes (2026-09-03, sempre "—" nas linhas da Análise de Balanço); tabela/modal continuam com **9 colunas** (Cliente, Grupo, Unidade, Segmento, Gerente de Contas, Tributação, Status, Documentação, Documento Pendente).
...
### Placares e abas Pendente/Concluído (2026-09-18: Radar Fiscal volta a seguir o Status, não a Documentação)
**Histórico**: em 2026-09-02 os placares mediam o FECHAMENTO (`Status === "Fechado"`); em 2026-09-03 passaram a seguir a coluna "Documentação" (`documentacaoRecebida`), pra sempre bater com o que a coluna mostra. Em 2026-09-18 o usuário separou os dois conceitos de novo, especificamente pro Radar Fiscal: **"a documentação não define qual a aba correta"** — Status "Simulando" fica na aba Pendente mesmo já estando com Documentação "Recebida" (só "Não importado" tem Documentação Pendente de verdade; "Simulando" também é considerado documentação recebida, mas ainda pendente de fechamento).

Os 3 placares do topo (`renderizarPlacares`): **Total de Empresas / Documentação Recebida / Documentação Pendente**. O corte é `documentacaoRecebida(r)`, que agora bifurca por `TipoRelatorio`:
- **Radar Fiscal**: `!STATUS_PENDENTE_RADAR_FISCAL.has(r.Status)` — `Set(["Não importado", "Simulando"])` = Pendente; `Fechado`/`Bloqueado`/`Com o GC` = Concluído. Ignora a coluna `Documentacao`.
- **Análise de Balanço**: continua `r.Documentacao === "Documentação Recebida"` (tarefa de retorno do checklist em aberto, sem relação com Status).

Recebida/Pendente são clicáveis: trocam a aba da tabela do fim (mesmo `data-aba` "Concluído"/"Pendente" — só os rótulos dos placares mudaram) e rolam até ela.

`#tabela-abas`: `abaTabelaAtiva` ("Pendente"|"Concluído") é um corte em `aplicarFiltroTabela()` **depois** dos 6 filtros: Pendente = `!documentacaoRecebida(r)`, Concluído = `documentacaoRecebida(r)` — mesma função bifurcada acima, então placar e aba sempre batem com o mesmo número. `definirAbaTabela()` (reset, volta pra "Pendente") vs `trocarAbaTabela()` (troca + re-renderiza). Os `data-aba` internos continuam "Pendente"/"Concluído"; os `<button>` no HTML seguem rotulados "Pendente"/"Concluído".
...
Duas colunas novas na tabela/modal, alimentadas de fora dos robôs deste projeto: o **Relatório de Distribuição** (Intranet > MG Controle > Relatórios > "Rel. Distribuição"), planilha pesada (~6,4 mil clientes × 44 colunas) baixada só ~1x/mês pelo hub [[project_atualizacao_de_bases]] (relatório `rel_distribuicao`, botão manual, fora do "Roda Tudo" — `REPORTS_CONFIG["run_all"]=False`). O extrator reduz o bruto pro cache slim `ID`/`Prioridade`/`DocumentosSituacao` (dedup — matriz e filial têm `ID` próprio cada uma, não precisa consolidar) e grava direto em `data/relatorio_fechamentos/distribuicao.xlsx` — gitignorado (mesma exceção de sempre, só libera o `resumo.xlsx`), nunca versionado.

`backend/orquestrador.py::_juntar_distribuicao(df, log)` faz o LEFT JOIN por **`Id`** (bate 100% com a coluna `ID` do relatório, validado 2026-09-04 — Radar Fiscal usa `IdCorporativo`, Análise de Balanço `IdCliente`, ambos já viram `Id` no schema normalizado), chamado logo antes de `_escrever_resumo_portal(df)`. Não-bloqueante: cache ausente = colunas vazias.

**Pegadinha real encontrada nesta migração**: `normalizarLinha()` no front é um allowlist explícito de campos — esquecer de adicionar `Prioridade`/`DocumentosSituacao` nela faz o SheetJS ler as colunas certinho do `resumo.xlsx` mas `carregarDados()` as descarta silenciosamente (sem erro no console, sem exceção — só "—" na tabela pra sempre). Confirmado com `fetch`+`XLSX.read` manual no console mostrando os dados corretos enquanto `dados` (populado via `normalizarLinha`) não tinha o campo. **Lição**: qualquer coluna nova que passe a existir no `resumo.xlsx` só chega no front se `normalizarLinha()` for atualizada também — não é automático como em portais que fazem `sheet_to_json` sem allowlist (ex. [[project_analise_entrega_sped]]).

Front: `linhaTabelaHTML()` ganhou `<td>${celula(r.Prioridade)}</td>`/`<td>${celula(r.DocumentosSituacao)}</td>` (entre Tributação e Status); `<colgroup>`/`<thead>` das duas tabelas viraram **11 colunas**. Filtros Prioridade/Doc. Situação adicionados nos 3 blocos (`f-*`/`t-*`/`m-*`) — mesmo padrão dos outros: `el.prioridade`/`el.docSituacao` (+ `t`/`m`), `filtrarConjunto()`, `CAMPO_PARA_FILTROS`, `sincronizarFiltroTabelaComGeral()`, `limparFiltrosGerais()`/`limparFiltrosTabela()`, repopulate em `atualizarCorpoDashboard()`/`abrirModal()`. Validado ao vivo via JS direto no console (dados reais: 3599/4222 com Prioridade, 1498/4222 com DocumentosSituacao).
